...
void setupUDP() {
  ...
  if (udp2.listen(PORT2)) {
    ...
    udp2.onPacket([](AsyncUDPPacket& packet) {
      csharpIP = packet.remoteIP();
      csharpPort = packet.remotePort();

      if (debugPwmLevel == 12) {
        Serial.print("udp2 RX from ");
        Serial.print(csharpIP);
        Serial.print(":");
        Serial.println(csharpPort);
      }
...
void handleUDPMessage(const uint8_t* data, size_t len) {
  ...
  if (strncmp(message, "UPDATE_SETTINGS", strlen("UPDATE_SETTINGS")) == 0) {
    ...
    double Kp, Ki, Kd;
    char Unit[9];
    uint8_t PWM_Conventional, Stagger, debugPwmLevel;
    ...
    if (parsedFields != 23) {
      Serial.print("UPDATE_SETTINGS parse FAILED. parsedFields=");
    ...
    // Detect target change BEFORE overwriting
    //bool newTarget = fabsf(PressureTarget - userSettings.PressureTarget) > TARGET_EPS;
...
void OnNewPressureTargetReceived() {
  pressureControlActive = true;
  settleCounter = 0;

  // If using PID mode, reset it so you don't carry integral across targets
  myPID.SetMode(MANUAL);
  myPID.SetMode(AUTOMATIC);
}
...
void sendSensorData() {
  ...
  if (len <= 0 || len >= (int)sizeof(buffer)) {
    Serial.println("Error formatting sensor data.");
    return;
  }

  if (csharpPort == 0) {
    Serial.println("No C# endpoint known yet; cannot send sensor data.");
    return;
  }

  // ✅ Unicast reply (reliable)
  if (!udp2.writeTo((const uint8_t*)buffer, len, csharpIP, csharpPort)) {
    Serial.print("Failed to unicast sensor data to ");
    Serial.print(csharpIP);
    Serial.print(":");
    Serial.println(csharpPort);
  } else {
    if (debugPwmLevel == 12) Serial.println("Sensor data unicasted successfully.");
  }
}
