...
void BasicPWM(size_t nozzleIndex, uint8_t dutyPercent = 50);
// Forward declarations
void ControlNozzles(size_t nozzleIndex, uint32_t cycleTimeMs);
void EvenOdd(size_t nozzleIndex, bool globalEvenNozzlesActive);

// --- Option 1 single-writer valve outputs ---
extern bool sectionState[64];
void writeSectionPhysical(uint8_t sectionIdx, bool isOnLogical);
...
void PWM_Controls() {
  ...
  // Build desired nozzle state from AOG 64-section bitmap plus any legacy pinStates mapping.
  bool desiredNozzleLocal[64];
  for (uint8_t n = 0; n < 64; n++) {
    desiredNozzleLocal[n] = sectionState[n];
  }

  // Merge in pinStates (fn 1..maxNozzles). OR-merge: either source can turn a nozzle ON.
  for (size_t i = 0; i < pinStates.size(); i++) {
    uint8_t fn = pinStates[i].fn;
    if (fn >= 1 && fn <= maxNozzles) {
      uint8_t idx = (uint8_t)(fn - 1);
      if (idx < 64) desiredNozzleLocal[idx] = desiredNozzleLocal[idx] || pinStates[i].state;
    }
  }

  // Count active nozzles (for boom math + downstream)
  uint8_t activeCount = 0;
  for (uint8_t n = 0; n < maxNozzles && n < 64; n++) {
    if (desiredNozzleLocal[n]) activeCount++;
  }
  numActiveNozzles = activeCount;

  boomLength = (numActiveNozzles * userSettings.SprayWidth);

  // Drive ALL nozzles from the desired state (single writer).
  for (uint8_t nozzleIndex = 0; nozzleIndex < maxNozzles && nozzleIndex < 64; nozzleIndex++) {

    bool isNozzleEnabled = desiredNozzleLocal[nozzleIndex];
    ...
    if (isNozzleEnabled) {
      ...
      if (userSettings.PWM_Conventional == 0) {
        writeSectionPhysical(nozzleIndex, true);
      }

    } else {
      writeSectionPhysical(nozzleIndex, false);
    }
  }
}
...
void ControlNozzles(size_t nozzleIndex, uint32_t cycleTimeMs) {
  ...
  if (perMs == 0 || nozzleIndex >= maxNozzles) {
    writeSectionPhysical((uint8_t)nozzleIndex, false);
    return;
  }
  ...
  if (debugPwmLevel == 99) {
    writeSectionPhysical((uint8_t)nozzleIndex, inOnWindow);
    return;
  }
...
void NozzleSpeed(size_t nozzleIndex) {
  ...
  // Calculate the distance from the boom's center for this nozzle.
    // Distance of this nozzle from boom center (in spray-width units)
  float centerIndex = ((float)maxNozzles - 1.0f) / 2.0f;
  float distanceFromCenter = ((float)nozzleIndex - centerIndex) * userSettings.SprayWidth; // same units as SprayWidth
  ...
  dutycycleTurncomp((uint8_t)nozzleIndex, nozzleSpeedMph);
}


void dutycycleTurncomp(uint8_t nozzleIndex, float nozzleSpeedMph) {
